...
def get_single_query_page(path, cluster, page_cursor=None, page_length=1000):
    params = []
    if page_cursor:
...
def check_cluster_connectivity(cluster):
    return do_request(HTTP_GET, "/ws.v1/control-cluster",
                      cluster=cluster)


def get_gateway_services(cluster):
    return do_request(HTTP_GET, "/ws.v1/gateway-service?fields=uuid",
                      cluster=cluster)


def get_transport_zones(cluster):
    return do_request(HTTP_GET, "/ws.v1/transport-zone?fields=uuid",
                      cluster=cluster)
