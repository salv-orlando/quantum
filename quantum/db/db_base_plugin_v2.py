...
class QuantumDbPluginV2(quantum_plugin_base_v2.QuantumPluginBaseV2):
    ...
    def _model_query(self, context, model):
        ...
        for hook_name, hooks in self._model_query_hooks.get(model,
                                                            {}).iteritems():
            LOG.debug("### Applying hook:%s" % hook_name)
        ...
        LOG.debug("### Query after applying filter:%s", query)
        return query
    ...
    def register_model_query_hook(cls, model, name, query_hook, filter_hook):
        ...
        """
        LOG.debug("### Registering hook for model:%s", model)
        model_hooks = cls._model_query_hooks.get(model)
        if not model_hooks:
            # add key to dict
            LOG.debug("### must add model hook key")
            model_hooks = {}
            cls._model_query_hooks[model] = model_hooks
        model_hooks[name] = {'query': query_hook, 'filter': filter_hook}
        LOG.debug("### verify model hook list:%s",
                  cls._model_query_hooks.get(model))
        
    @classmethod
    def unregister_model_query_hook(cls, model, name):
        """ Remove the specified hook for the selected model """
        if model in cls._model_query_hooks:
            if name in cls._model_query_hooks[model]:
                LOG.debug("### Removing entry %s from dict for model:%s", name, model)
                del cls._model_query_hooks[model][name]
                if len(cls._model_query_hooks[model]) == 0:
                    LOG.debug("### Removing entry from main hook map")
                    del cls._model_query_hooks[model]
                LOG.debug("Model hook %s for model %s unregistered",
                          name, model)
                return
            else:
                # No need to raise at the moment, might do in the future
                LOG.warning("Unable to find hook %s for model %s",
                            name, model)
        else:
            # No need to raise at the moment, might do in the future
            LOG.warning("No hook registered for model %s" % model)
